...
function maxNonAdjacentSum(nums, memo = {}) {
  ...
  table[0] = nums[0];
  for (let i = 0; i < table.length; i++) {
    let jumpLeft = table[i - 2] === undefined ? 0 : table[i - 2];
    let bestStepNum = nums[i] + jumpLeft;
    let bestStepNoNum = table[i - 1];
    table[i] = Math.max(bestStepNum, bestStepNoNum);     
  };
  return table[table.length -1];
}
...
function minChange(coins, amount) {
    ...
    coins.forEach(element => {
        for (let i = 0; i < table.length; i++) {
            for (let j = 0; j * element <= i; j++) {
                reminder = i - j * element;
                potentialS = table[reminder] + j;
                if (potentialS < table[i]){
                    table[i] = potentialS;
                }   
            }
